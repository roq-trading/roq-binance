...
template <>
struct fmt::formatter<roq::binance::json::L2Update::change_t> {
  template <typename C>
  constexpr auto parse(C& ctx) {
    return ctx.begin();
  }
  template <typename C>
  auto format(const roq::binance::json::L2Update::change_t& value, C& ctx) {
    return format_to(
        ctx.out(),
        "{{"
        "price={}, "
...
template <>
struct fmt::formatter<roq::binance::json::L2Update> {
  template <typename C>
  constexpr auto parse(C& ctx) {
    return ctx.begin();
  }
  template <typename C>
  auto format(const roq::binance::json::L2Update& value, C& ctx) {
    return format_to(
        ctx.out(),
        "{{"
        "changes=[{}], "
