...
uint32_t Rest::download(State state) {
  switch (state) {
    ...
    case UNDEFINED:
      ...
      break;
    case EXCHANGE_INFO:
      (*this)(ConnectionStatus::DOWNLOADING, "exchange-info");
      get_exchange_info();
      return 1;
    case DONE:
...
void Rest::operator()(Trace<protocol::json::ExchangeInfoAck> const &event) {
        ...
        switch (item.interval) {
          ...
          case DAY:
            if (item.interval_num == 1) {
              shared_.limits.create_order_1d = item.limit;
            } else {
              log::warn("Unknown interval: rate_limit={}"sv, item);
            }
            break;
  ...
  for (auto &item : exchange_info_ack.symbols) {
    log::info<2>("item={}"sv, item);
    auto discard = shared_.dispatcher.discard_symbol(item.symbol);  // XXX should this be normalized symbol ???
    // fall-back values
