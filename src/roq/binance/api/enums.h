...
template <>
struct fmt::formatter<roq::binance::api::Side> {
  template <typename C>
  constexpr auto parse(C& ctx) {
    return ctx.begin();
  }
  template <typename C>
  auto format(const roq::binance::api::Side& value, C& ctx) {
    return format_to(
        ctx.out(),
        "{}",
        roq::binance::api::EnumNameSide(value));
  }
};

template <>
struct fmt::formatter<roq::binance::api::OrderType> {
  template <typename C>
  constexpr auto parse(C& ctx) {
    return ctx.begin();
  }
  template <typename C>
  auto format(const roq::binance::api::OrderType& value, C& ctx) {
    return format_to(
        ctx.out(),
        "{}",
        roq::binance::api::EnumNameOrderType(value));
  }
};

template <>
struct fmt::formatter<roq::binance::api::OrderStatus> {
  template <typename C>
  constexpr auto parse(C& ctx) {
    return ctx.begin();
  }
  template <typename C>
  auto format(const roq::binance::api::OrderStatus& value, C& ctx) {
    return format_to(
        ctx.out(),
        "{}",
        roq::binance::api::EnumNameOrderStatus(value));
  }
};

template <>
struct fmt::formatter<roq::binance::api::TimeInForce> {
  template <typename C>
  constexpr auto parse(C& ctx) {
    return ctx.begin();
  }
  template <typename C>
  auto format(const roq::binance::api::TimeInForce& value, C& ctx) {
    return format_to(
        ctx.out(),
        "{}",
        roq::binance::api::EnumNameTimeInForce(value));
  }
};

template <>
struct fmt::formatter<roq::binance::api::Reason> {
  template <typename C>
  constexpr auto parse(C& ctx) {
    return ctx.begin();
  }
  template <typename C>
  auto format(const roq::binance::api::Reason& value, C& ctx) {
    return format_to(
        ctx.out(),
        "{}",
        roq::binance::api::EnumNameReason(value));
  }
};

template <>
struct fmt::formatter<roq::binance::api::XStatus> {
  template <typename C>
  constexpr auto parse(C& ctx) {
    return ctx.begin();
  }
  template <typename C>
  auto format(const roq::binance::api::XStatus& value, C& ctx) {
    return format_to(
        ctx.out(),
        "{}",
        roq::binance::api::EnumNameStatus(value));
  }
};

template <>
struct fmt::formatter<roq::binance::api::StopType> {
  template <typename C>
  constexpr auto parse(C& ctx) {
    return ctx.begin();
  }
  template <typename C>
  auto format(const roq::binance::api::StopType& value, C& ctx) {
    return format_to(
        ctx.out(),
        "{}",
        roq::binance::api::EnumNameStopType(value));
  }
};
